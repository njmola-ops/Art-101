...
$("#ships1").hover(function () {
  $("#output").html(
    "<p class='scan-text1'>55 Cacri e, the 'diamond planet', is mostly composed of carbon!</p>"
  );
});

$("#ships2").hover(function () {
  $("#output").html(
    "<p class='scan-text1'>The Pillars of Creation, located in the Eagle Nebula, is a region of active star formation!</p>"
  );
});

$("#ships3").hover(function () {
  $("#output").html(
    "<p class='scan-text1'>KELT-9b is the hottest exoplanet known to man!</p>"
  );
});

$("#ships4").hover(function () {
  $("#output").html(
    "<p class='scan-text1'>I don't know why they would ever go there.</p>"
  );
});
});
...
const shipDestinations = {
  ships1: "Destination: 55 Cancri e",
  ships2: "Destination: The Pillars of Creation",
  ships3: "Destination: KELT-9b",
  ships4: "Destination: Porter Dining Hall"
};


const $tooltip = $("<div class='ship-tooltip'></div>");
$("body").append($tooltip);

$(".shipz img").hover(
  function () {
    const id = $(this).attr("id");
    const text = shipDestinations[id] || "Destination unknown";
...
$tooltip
  .text(text)
  .css({
    left: shipRect.left + window.scrollX + shipRect.width / 2,
    top: shipRect.bottom + window.scrollY + 5,
    transform: "translateX(-50%)"
  })
  .stop(true, true)
  .fadeIn(150);
    
    $("#status").text("Scanning ship trajectory...");
  },
  function () {
...
$(".shipz img").on("mousedown", function (e) {
  dragging = true;
  currentShip = $(this);

  const rect = this.getBoundingClientRect();
  const containerRect = $(".ship-container")[0].getBoundingClientRect();
...
$(document).on("mousemove", function (e) {
  if (!dragging || !currentShip) return;

  const containerOffset = $(".ship-container").offset();

  currentShip.css({
    left: e.pageX - containerOffset.left - offsetX,
    top: e.pageY - containerOffset.top - offsetY
  });
});
...
const originalShipState = [];

$(".shipz img").each(function (index) {
  const $img = $(this);

  originalShipState[index] = {
    element: $img,
    src: $img.attr("src"),
    id: $img.attr("id")
  };
...
function resetShips() {
  $(".shipz img").each(function (index) {
    const original = originalShipState[index];
